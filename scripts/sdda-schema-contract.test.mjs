...
const componentMoveUpMigration = readFileSync(
  new URL('../supabase/sdda-migrations/20260813_0012_component_move_up.sql', import.meta.url),
  'utf8'
).toLowerCase();
const multilevelRunsMigration = readFileSync(
  new URL(
    '../supabase/sdda-migrations/20260813_0013_multilevel_component_runs.sql',
    import.meta.url
  ),
  'utf8'
).toLowerCase();
const publicEntryMigration = readFileSync(
  new URL(
    '../supabase/sdda-migrations/20260813_0014_public_entry_confirmations.sql',
    import.meta.url
  ),
  'utf8'
).toLowerCase();
const publicEntryRandomMigration = readFileSync(
  new URL(
    '../supabase/sdda-migrations/20260813_0015_public_entry_random_bytes.sql',
    import.meta.url
  ),
  'utf8'
).toLowerCase();
const publicEntryGroupsMigration = readFileSync(
  new URL(
    '../supabase/sdda-migrations/20260813_0016_public_entry_run_group_requests.sql',
    import.meta.url
  ),
  'utf8'
).toLowerCase();
const perRunStreamsMigration = readFileSync(
  new URL('../supabase/sdda-migrations/20260813_0017_per_run_entry_streams.sql', import.meta.url),
  'utf8'
).toLowerCase();
const gamesMigration = readFileSync(
  new URL('../supabase/sdda-migrations/20260819_0018_trial_formats_and_games.sql', import.meta.url),
  'utf8'
).toLowerCase();
const publicGamesEntryMigration = readFileSync(
  new URL('../supabase/sdda-migrations/20260820_0019_public_games_entries.sql', import.meta.url),
  'utf8'
).toLowerCase();
const secureFormattedTrialMigration = readFileSync(
  new URL(
    '../supabase/sdda-migrations/20260820_0020_secure_formatted_trial_creation.sql',
    import.meta.url
  ),
  'utf8'
).toLowerCase();
const entryEditingMigration = readFileSync(
  new URL(
    '../supabase/sdda-migrations/20260820_0021_entry_editing_and_aerial.sql',
    import.meta.url
  ),
  'utf8'
).toLowerCase();
...
test('encodes SDDA days, levels, components, streams, groups, and pending registration', () => {
  ...
  for (const value of ['started', 'advanced', 'excellent', 'elite'])
    assert.match(migration, new RegExp(`'${value}'`));
  for (const value of ['container', 'interior', 'exterior'])
    assert.match(migration, new RegExp(`'${value}'`));
...
test('prevents cross-trial runs and binds sensitive writes to the caller', () => {
  assert.match(
    migration,
    /foreign key \(entry_id, trial_id\)[\s\S]*references public\.sdda_entries\(id, trial_id\)/
  );
  assert.match(
    migration,
    /foreign key \(trial_day_id, trial_id\)[\s\S]*references public\.sdda_trial_days\(id, trial_id\)/
  );
...
test('keeps assistants and viewers out of financial records', () => {
  const functionStart = migration.indexOf(
    'create or replace function public.sdda_can_manage_finances'
  );
...
test('secures atomic trial creation without exposing anonymous execution', () => {
  assert.match(
    secureTrialCreationMigration,
    /alter function public\.sdda_create_trial.*security definer/s
  );
...
test('imports SDDA entries atomically only into configured offerings', () => {
  ...
  assert.match(
    entryImportMigration,
    /revoke all on function public\.sdda_import_entry.*from anon/s
  );
...
test('reuses one entry while importing component-specific runs idempotently', () => {
  assert.match(
    unifiedEntryRunsMigration,
    /alter table public\.sdda_runs\s+add column if not exists stream/
  );
  assert.match(
    unifiedEntryRunsMigration,
    /update public\.sdda_runs r\s+set stream\s*=\s*e\.stream/s
  );
  assert.match(unifiedEntryRunsMigration, /target_entry_id/);
  assert.match(unifiedEntryRunsMigration, /from public\.sdda_entries/);
  assert.match(
    unifiedEntryRunsMigration,
    /on conflict\s*\(entry_id,\s*trial_day_id,\s*component\)\s*do update/
  );
...
test('supports audited component move-ups on any offered trial day', () => {
  ...
  assert.doesNotMatch(
    componentMoveUpMigration,
    /day_number\s*<=\s*1|service_role|security definer|cwags|c-wags/
  );
});
...
test('accepts validated competitor running-group requests while keeping table access private', () => {
  for (const value of ['official', 'regular', 'second dog', 'feo', 'bis'])
    assert.match(publicEntryGroupsMigration, new RegExp(`'${value}'`));
  assert.match(publicEntryGroupsMigration, /sdda_submit_public_entry_core/);
  assert.match(
    publicEntryGroupsMigration,
    /revoke all on function public\.sdda_submit_public_entry_core.*from public, anon, authenticated/
  );
...
test('models Scent, Games, and Combined trials without reusing scent run columns', () => {
  ...
  for (const game of ['aerial', 'distance', 'speed', 'team'])
    assert.match(gamesMigration, new RegExp(`'${game}'`));
...
test('accepts public Games entries without anonymous table access', () => {
  ...
  assert.match(
    publicGamesEntryMigration,
    /grant execute on function public\.sdda_submit_public_entry/
  );
  assert.doesNotMatch(
    publicGamesEntryMigration,
...
test('creates formatted trials through the authenticated RLS boundary', () => {
  assert.match(
    secureFormattedTrialMigration,
    /alter function public\.sdda_create_trial\(text,text,text,date\[\],text\)\s+security definer/
  );
...
test('supports audited token-scoped and secretary entry editing with Aerial categories', () => {
  assert.match(entryEditingMigration, /aerial_division.*'high','highfly'/s);
  assert.match(entryEditingMigration, /function public\.sdda_update_public_entry/);
  assert.match(
    entryEditingMigration,
    /receipt_token_hash=encode\(extensions\.digest\(receipt_token,'sha256'\),'hex'\)/
  );
  assert.match(entryEditingMigration, /confirmation_status='received'.*status='entries_open'/s);
  assert.match(entryEditingMigration, /function public\.sdda_update_entry_as_secretary/);
  assert.match(entryEditingMigration, /sdda_can_manage_trial\(target_trial_id\)/);
  assert.match(entryEditingMigration, /entry\.public_updated/);
  assert.match(entryEditingMigration, /entry\.secretary_updated/);
  assert.match(entryEditingMigration, /before_state,after_state/);
  assert.match(
    entryEditingMigration,
    /revoke all on function public\.sdda_apply_entry_update.*from public,anon,authenticated/
  );
  assert.doesNotMatch(
    entryEditingMigration,
    /grant (select|insert|update|delete).*to anon|service_role|cwags|c-wags/
