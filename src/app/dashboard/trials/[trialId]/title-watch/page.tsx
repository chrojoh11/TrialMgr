...
import { AlertTriangle, Database, FileSpreadsheet, Loader2, Printer, RefreshCw, Search, Trophy } from 'lucide-react';
...
export default function SddaTitleWatchPage() {
  ...
  return <MainLayout title="Title watch" breadcrumbItems={[
    ...
    <Card className="overflow-hidden"><CardHeader className="bg-gradient-to-r from-[#225f45] to-[#173f30] text-white"><div className="flex flex-wrap items-center justify-between gap-4"><div className="flex items-center gap-3"><Trophy className="h-8 w-8 text-[#f1cf72]" /><div><CardTitle className="text-2xl">Close to Titles Report</CardTitle><CardDescription className="text-emerald-100">{trial?.name}</CardDescription></div></div><div className="flex flex-wrap gap-2 print:hidden"><button type="button" onClick={() => void exportTitleReport()} className="flex items-center rounded-md border border-white/50 bg-white px-3 py-2 text-sm font-semibold text-[#225f45]"><FileSpreadsheet className="mr-2 h-4 w-4" />Export Excel</button><button type="button" onClick={() => window.print()} className="flex items-center rounded-md border border-white/50 bg-white px-3 py-2 text-sm font-semibold text-[#225f45]"><Printer className="mr-2 h-4 w-4" />Print / Save PDF</button></div></div></CardHeader><CardContent className="space-y-5 p-6"><Alert className="border-blue-200 bg-blue-50"><AlertTriangle className="h-4 w-4 text-blue-700" /><AlertDescription className="text-blue-950"><strong>Maximum-outcome assumption:</strong> every required component entered at this trial qualifies. Confirm actual results before presenting titles or ribbons.</AlertDescription></Alert><div className="grid gap-4 sm:grid-cols-3"><div className="rounded-lg border-2 border-violet-200 bg-violet-50 p-4"><p className="text-sm text-gray-600">Possible titles</p><p className="text-3xl font-bold text-violet-700">{potentialAwards.length}</p></div><div className="rounded-lg border-2 border-orange-200 bg-orange-50 p-4"><p className="text-sm text-gray-600">Working reviews</p><p className="text-3xl font-bold text-orange-700">{workingReview.length}</p></div><div className="rounded-lg border-2 border-amber-200 bg-amber-50 p-4"><p className="text-sm text-gray-600">Maximum ribbons</p><p className="text-3xl font-bold text-amber-700">{potentialAwards.length}</p></div></div></CardContent></Card>
    ...
    {!loading && <Card><CardHeader><CardTitle>Possible titles and ribbon preparation</CardTitle><CardDescription>Grouped by SDDA level, following the report organization used successfully in C-WAGS.</CardDescription></CardHeader><CardContent className="space-y-5">{ribbonTotals.length > 0 ? <><div className="flex flex-wrap gap-2">{ribbonTotals.map(([titleName,count]) => <Badge key={titleName} className="bg-[#225f45] text-white">{titleName}: {count}</Badge>)}<Badge className="bg-[#b98935] text-white">Maximum total: {potentialAwards.length}</Badge></div>{awardsByLevel.map(({level,items}) => <details key={level} open className="rounded-lg border border-violet-200 bg-white"><summary className="cursor-pointer bg-violet-50 px-4 py-3 text-lg font-bold text-violet-800">{level} · {items.length} possible title{items.length === 1 ? '' : 's'}</summary><div className="space-y-3 p-4">{items.map(({entry,dog,award}) => <div key={`${entry.id}-${award.title}`} className="rounded-lg border border-violet-200 bg-violet-50 p-4 break-inside-avoid"><div className="flex flex-wrap items-start justify-between gap-3"><div><p className="text-lg font-bold">{dog?.call_name || 'Dog'}</p><p className="text-sm text-gray-600">{entry.handler_name} · {dog?.sdda_registration_number || 'Registration pending'}</p></div><Badge className="bg-violet-700 text-white">{award.title}</Badge></div><div className="mt-3 grid gap-3 text-sm sm:grid-cols-3"><div className="rounded bg-white p-3"><p className="text-xs font-semibold uppercase text-gray-500">Must qualify in</p><p className="mt-1 font-bold text-emerald-700">{award.requiredComponents.join(', ')}</p></div><div className="rounded bg-white p-3"><p className="text-xs font-semibold uppercase text-gray-500">Entered components</p><p className="mt-1 font-bold">{[...new Set((entry.sdda_runs || []).filter((run:any) => run.level === award.level).map((run:any) => run.component))].join(', ')}</p></div><div className="rounded bg-white p-3"><p className="text-xs font-semibold uppercase text-gray-500">Verification</p><p className="mt-1">{award.historyVerified ? 'Official component counts compared' : 'Verify prior Elite status manually'}</p></div></div></div>)}</div></details>)}</> : <p className="rounded-md border bg-[#f7f8f4] p-4 text-gray-600">No title ribbons are currently projected from the entered runs and official component history.</p>}</CardContent></Card>}
  </div></MainLayout>;
}
