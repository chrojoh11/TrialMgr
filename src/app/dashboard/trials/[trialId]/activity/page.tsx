...
import { activityFieldLabel, displayActivityValue, initialTrialSetupRecordIds, secretaryActivityChanges } from '@/lib/sdda/activityPresentation';
...
export default function SddaActivityPage() {
  ...
  const filtered = useMemo(() => records.filter((item: any) => {
  ...
  }), [records, search, type, initialSetupIds]);
  return <MainLayout title="Activity journal" breadcrumbItems={[{ label: 'Dashboard', href: '/dashboard' }, { label: 'Trials', href: '/dashboard/trials' }, { label: trial?.name || 'Trial', href: `/dashboard/trials/${trialId}` }, { label: 'Activity' }]}><div className="mx-auto max-w-6xl space-y-6">
    ...
    {loading ? <div className="flex justify-center py-20"><Loader2 className="h-8 w-8 animate-spin" /></div> : <div className="space-y-6">{filtered.map((item: any) => {
      ...
      return <article key={item.id} className="overflow-hidden rounded-sm border border-gray-300 bg-white shadow-sm print:break-inside-avoid print:shadow-none">
        <header className="border-b-2 border-[#225f45] px-6 py-5"><div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs font-bold uppercase tracking-[.14em] text-[#68736c]">SDDA TrialDesk activity statement</p><h2 className="mt-1 font-serif text-2xl font-bold text-[#18231d]">{item.action === 'trial.created' ? 'Trial Created And Initial Setup' : title(item.action)}</h2></div><time className="text-sm font-medium text-gray-600">{new Date(item.created_at).toLocaleString('en-CA')}</time></div></header>
