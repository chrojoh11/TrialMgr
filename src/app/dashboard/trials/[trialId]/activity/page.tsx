    ...
    {loading ? <div className="flex justify-center py-20"><Loader2 className="h-8 w-8 animate-spin" /></div> : <div className="space-y-6">{filtered.map((item: any) => {
      ...
      return <article key={item.id} className="overflow-hidden rounded-sm border border-gray-300 bg-white shadow-sm print:break-inside-avoid print:shadow-none">
        ...
        <div className="p-6"><h3 className="mb-3 text-sm font-bold uppercase tracking-wider text-[#225f45]">{changes.length ? `${changes.length} recorded change${changes.length === 1 ? '' : 's'}` : 'Recorded event'}</h3>
          ...
          {(item.before_state || item.after_state) && <details className="mt-4 print:hidden"><summary className="cursor-pointer text-sm font-semibold text-[#225f45]">Technical audit snapshot</summary><div className="mt-3 grid gap-3 md:grid-cols-2">{item.before_state && <div className="rounded border bg-gray-50 p-3"><p className="mb-1 font-medium">Before</p><pre className="overflow-auto whitespace-pre-wrap text-xs">{JSON.stringify(item.before_state, null, 2)}</pre></div>}{item.after_state && <div className="rounded border bg-gray-50 p-3"><p className="mb-1 font-medium">After</p><pre className="overflow-auto whitespace-pre-wrap text-xs">{JSON.stringify(item.after_state, null, 2)}</pre></div>}</div></details>}
        </div>
      </article>;
