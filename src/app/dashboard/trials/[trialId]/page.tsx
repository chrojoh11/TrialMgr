...
const localDateTimeInput = (value: string | null, timeZone: string) => {
  if (!value) return '';
...
const trialLocalDisplay = (value: string, timeZone: string) => new Intl.DateTimeFormat('en-CA', {
...
export default function SddaTrialWorkspacePage() {
  ...
  const [publicDetails, setPublicDetails] = useState({ secretaryName: '', secretaryEmail: '', secretaryPhone: '', paymentInstructions: '', cancellationPolicy: '', generalOpenAt: '', closeAt: '', timeZone: 'America/Edmonton' });
  ...
  const entrySchedule = (() => {
    if (!trial || trial.status !== 'entries_open') return { label: 'Manually closed', accepting: false };
    if (!trial.entry_open_at && !trial.entry_close_at) return { label: 'Open manually', accepting: true };
    const now = Date.now();
    const registeredAt = trial.entry_open_at ? new Date(trial.entry_open_at).getTime() : Number.NEGATIVE_INFINITY;
    const generalAt = trial.general_entry_open_at ? new Date(trial.general_entry_open_at).getTime() : registeredAt;
    const closesAt = trial.entry_close_at ? new Date(trial.entry_close_at).getTime() : Number.POSITIVE_INFINITY;
    if (now < registeredAt) return { label: 'Scheduled—not open yet', accepting: false };
    if (now > closesAt) return { label: 'Automatically closed', accepting: false };
    if (now < generalAt) return { label: 'Registered participants only', accepting: true };
    return { label: 'Open to everyone', accepting: true };
  })();
  const entryScheduleConfigured = Boolean(publicDetails.generalOpenAt && publicDetails.closeAt);
    ...
    try {
      ...
      if (publicDetails.generalOpenAt || publicDetails.closeAt) {
        await saveSddaTrialEntrySchedule(
          getSupabaseBrowser(),
          trial.id,
          publicDetails.generalOpenAt,
          publicDetails.closeAt,
          publicDetails.timeZone,
        );
      }
      await load(); setSaved(true);
    } catch (caught) {
    ...
    <MainLayout breadcrumbItems={[{ label: 'Dashboard', href: '/dashboard' }, { label: 'Trials', href: '/dashboard/trials' }, { label: trial.name }]}>
      <div className="mx-auto max-w-7xl space-y-6">
        ...
        <Card><CardHeader><CardTitle>Secretary setup checklist</CardTitle><CardDescription>Complete the required setup first; trial numbers and judge assignments can remain pending until SDDA confirms them.</CardDescription></CardHeader><CardContent className="grid gap-3 md:grid-cols-2">{[
          ...
          { ready: entryScheduleConfigured, label: 'Automatic entry schedule', detail: entryScheduleConfigured ? entrySchedule.label : 'Set general opening and closing times; registered participants open three days early.' },
        ...
        {entrySchedule.accepting && (!dayAssignmentsComplete || !scentPricingReady || !gamesPricingReady || !competitorDetailsReady) && <Alert><AlertTriangle className="h-4 w-4" /><AlertDescription>Public entries are being accepted while some setup remains pending. Complete the highlighted checklist items before producing final running orders, judge packets, and financial reports.</AlertDescription></Alert>}
