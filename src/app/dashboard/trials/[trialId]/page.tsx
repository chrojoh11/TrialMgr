  ...
  return (
    <MainLayout breadcrumbItems={[{ label: 'Dashboard', href: '/dashboard' }, { label: 'Trials', href: '/dashboard/trials' }, { label: trial.name }]}>
      <div className="mx-auto max-w-7xl space-y-6">
        <div className="sticky top-0 z-30 -mx-2 flex flex-col gap-3 border-b border-[#c8d7cd] bg-[#fffdf7]/95 px-4 py-4 shadow-sm backdrop-blur sm:flex-row sm:items-start sm:justify-between lg:-mx-4">
          ...
          <Button onClick={save} disabled={(!dirty && !gamesDirty && !gameConfigurationDirty && !scentConfigurationDirty && !pricingDirty) || saving}>{saving ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Save className="mr-2 h-4 w-4" />}Save setup</Button>
        </div>
        ...
        {hasScent && <Card><CardHeader><CardTitle>Scent entry pricing</CardTitle><CardDescription>Trial-level fees used on the entry form, receipts, and in Finances. These are separate from public details and day assignments. A three-component price of $0 uses the individual component price three times.</CardDescription></CardHeader><CardContent className="grid gap-4 sm:grid-cols-3">
          ...
          <div className="sm:col-span-3"><Button type="button" variant="outline" disabled={!pricingDirty || savingPricing} onClick={() => void savePricing()}>{savingPricing ? <Loader2 className="mr-2 h-4 w-4 animate-spin" /> : <Save className="mr-2 h-4 w-4" />}Save Scent pricing</Button></div>
        </CardContent></Card>}
