...
import { Activity, Calendar, Check, ChevronDown, ChevronRight, CircleDollarSign, ClipboardList, Copy, FileSpreadsheet, FileText, Home, ListOrdered, Plus, Trophy, Users, X } from 'lucide-react';
...
export function Sidebar({ className = '', isMobileOpen = false, onCloseMobile }: SidebarProps) {
  ...
  const load = useCallback(async () => { try { setTrials((await listSddaTrials(getSupabaseBrowser())).slice(0, 5)); } catch (error) { console.error('Unable to load SDDA trials for navigation:', error); setTrials([]); } }, []);
  ...
  return <aside className={`${isMobileOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'} fixed inset-y-0 left-0 z-40 flex w-72 flex-col border-r border-[#cfd8d1] bg-[#f7f8f4] transition-transform lg:static ${className}`}>
    ...
    <nav className="flex-1 overflow-y-auto p-4">
      ...
      <div className="mt-6 border-t border-[#d9ded9] pt-4"><p className="mb-2 px-3 text-xs font-extrabold uppercase tracking-wider text-[#68736c]">Recent SDDA trials</p>{trials.length === 0 && <p className="px-3 py-2 text-sm text-[#68736c]">No trials yet.</p>}{trials.map((trial) => { const open = expanded.has(trial.id) || trial.id === activeTrialId; return <div key={trial.id} className="mb-1"><button type="button" onClick={() => toggle(trial.id)} className="flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left font-semibold text-[#35443a] hover:bg-white">{open ? <ChevronDown className="h-4 w-4" /> : <ChevronRight className="h-4 w-4" />}<span className="min-w-0 flex-1 truncate">{trial.name}</span></button>{open && <div className="ml-5 border-l border-[#cfd8d1] pl-2">{trialItems(trial.id).map((item) => <Link key={item.href} href={item.href} onClick={onCloseMobile} className={`flex items-center gap-2 rounded-md px-3 py-2 text-sm ${pathname === item.href ? 'bg-[#dfece4] font-bold text-[#225f45]' : 'text-[#526057] hover:bg-white'}`}><item.icon className="h-4 w-4" />{item.label}</Link>)}<button type="button" onClick={() => void copyEntryLink(trial.id)} className="flex w-full items-center gap-2 rounded-md px-3 py-2 text-sm text-[#526057] hover:bg-white">{copied === trial.id ? <Check className="h-4 w-4" /> : <Copy className="h-4 w-4" />}{copied === trial.id ? 'Entry link copied' : 'Copy SDDA entry link'}</button></div>}</div>; })}</div>
    </nav>
  </aside>;
}
