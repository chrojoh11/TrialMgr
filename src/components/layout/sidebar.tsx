...
export function Sidebar({ className = '', isMobileOpen = false, onCloseMobile }: SidebarProps) {
  ...
  const trialLink = (item: ReturnType<typeof beforeEntryLinks>[number]) => <Link key={item.href} href={item.href} onClick={onCloseMobile} className={`flex items-center gap-2 rounded-md px-3 py-2 text-sm ${pathname === item.href ? 'bg-[#dfece4] font-bold text-[#225f45]' : item.label === 'Financial Summary' ? 'bg-[#fff7df] font-semibold text-[#7a5718] hover:bg-[#ffefbd]' : 'text-[#526057] hover:bg-white'}`}><item.icon className="h-4 w-4 shrink-0" /><span>{item.label}</span></Link>;
    ...
    <nav className="flex-1 overflow-y-auto p-4">
      ...
      <section className="mt-5"><div className="mb-2 flex items-center justify-between px-3"><h2 className="text-xs font-extrabold uppercase tracking-wider text-[#68736c]">Trials</h2><Link href="/dashboard/trials" onClick={onCloseMobile} className="text-xs font-semibold text-[#225f45]">View all</Link></div>
        <div className="space-y-1">{visibleTrials.length === 0 && <p className="px-3 py-2 text-sm text-[#68736c]">No trials yet.</p>}{visibleTrials.map((trial) => { const open = expanded.has(trial.id) || trial.id === activeTrialId; const active = trial.id === activeTrialId; return <div key={trial.id}><button type="button" onClick={() => toggle(trial.id)} className={`flex w-full items-center gap-2 rounded-lg px-3 py-2 text-left font-semibold ${active ? 'bg-[#e9f1eb] text-[#225f45]' : 'text-[#35443a] hover:bg-white'}`}><Calendar className="h-4 w-4 shrink-0" /><span className="min-w-0 flex-1 truncate">{trial.name}</span>{open ? <ChevronDown className="h-4 w-4 shrink-0" /> : <ChevronRight className="h-4 w-4 shrink-0" />}</button>{open && <div className="ml-4 mt-1 space-y-1 border-l-2 border-[#cfd8d1] pl-2">
          ...
          <Link href={`/sdda-entry/${trial.id}`} target="_blank" onClick={onCloseMobile} className="flex items-center gap-2 rounded-md px-3 py-2 text-sm text-[#526057] hover:bg-white"><ExternalLink className="h-4 w-4 shrink-0" />Competitor Entry Form</Link>
          <button type="button" onClick={() => void copyEntryLink(trial.id)} className="flex w-full items-center gap-2 rounded-md px-3 py-2 text-left text-sm text-[#526057] hover:bg-white">{copied === trial.id ? <Check className="h-4 w-4 shrink-0 text-green-700" /> : <Copy className="h-4 w-4 shrink-0" />}<span className={copied === trial.id ? 'text-green-700' : ''}>{copied === trial.id ? 'Entry Link Copied' : 'Copy Entry Link'}</span></button>
          {afterEntryLinks(trial.id).map(trialLink)}
        </div>}</div>; })}
          <Link href="/dashboard/trials/create" onClick={onCloseMobile} className="mt-3 flex items-center gap-2 rounded-lg border-2 border-dashed border-[#b9cbbf] px-3 py-3 font-semibold text-[#225f45] hover:bg-white"><Plus className="h-4 w-4" />Create New Trial</Link>
        </div>
      </section>
    </nav>
  </aside>;
