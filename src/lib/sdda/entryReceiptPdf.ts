...
export type EntryReceiptPdfInput={confirmationCode:string;trialName:string;handlerName:string;dogName:string;runCount:number;selections:string[]};
export async function createEntryReceiptPdf(input:EntryReceiptPdfInput){
  ...
  line('CONFIRMATION NUMBER',10,bold);line(input.confirmationCode,22,bold);y-=8;line('Received - not yet accepted.',14,bold);line('The trial secretary must confirm this entry and provide payment instructions.');y-=10;line(input.trialName,18,bold);line(`${input.handlerName} with ${input.dogName}`);line(`${input.runCount} component run${input.runCount===1?'':'s'} requested`);y-=10;line('Selections',14,bold);
